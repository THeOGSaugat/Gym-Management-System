...
- **Phase 8 (notifications)** — done: in-app notifications for
  ...
  generated by the existing services at the moment each event happens
  (or, for the two membership ones, opportunistically on the same
  self-healing reads that already power the "How plans, memberships &
  payments work" self-heal pattern), with no cron job or queue. A bell
  in the header shows an unread count and links to a per-role
  notifications page to view, mark-as-read, mark-all-as-read, and open
...
6. Log in as `member@gym.test` → `/member/dashboard` → membership status
   ...
   "Notifications" shows "You're all caught up" (no seed data creates
   notifications directly — see "Manually test notifications" below for
   how to generate real ones) with a link through to the full
   notifications page.
...
## 13. Manually test notifications

No notification rows are seeded — they're only ever created by the
services that generate them, so you'll trigger some real ones first:

1. Log in as `admin@gym.test` and record a payment for any member (e.g.
   `member@gym.test`, via `/admin/members/[id]/payments/new`) → log in
   as that member → the header bell now shows an unread badge, and
   `/member/notifications` shows a "Payment recorded" notification with
   an "Open" link to `/member/payments`.
2. As `deepak.kapoor@gym.test`, create a new workout plan for
   `member@gym.test` (an already-assigned member) → log back in as
   `member@gym.test` → a new "New workout plan assigned" notification
   appears, its "Open" link going straight to that plan's detail page.
3. As `admin@gym.test`, reassign a member to a different trainer (e.g.
   move `member@gym.test` from Deepak to `hannah.weiss@gym.test` via
   that member's detail page) → log in as `member@gym.test` → a
   "Trainer assigned" notification appears. Log in as
   `hannah.weiss@gym.test` → a *different* "New member assigned"
   notification appears for her, linking to that member's roster page
   — each side of the relationship gets exactly one notification, not
   a copy of the other's.
4. On `/member/notifications`, click **Mark as read** on one
   notification → it visually dims and the header badge count drops by
   one. Click **Mark all as read** → every remaining notification dims
   and the badge disappears entirely.
5. Confirm isolation: log in as a *different* member who has no
   notifications of their own (e.g. `sofia.rossi@gym.test`, assuming
   you haven't triggered anything for her) → `/member/notifications`
   shows "No notifications yet" — never another member's rows, no
   matter which member triggered which event above.
6. To see the two membership-related notification types without
   waiting for a real membership to actually approach expiry, either
   assign a short-duration test plan and wait, or inspect the effect
   indirectly: any time a member's own membership page or dashboard
   self-heals a membership whose `endDate` is within 3 days (or that
   has just quietly expired), a "Membership expiring soon" or
   "Membership expired" notification appears — see "How notifications
   work" below for exactly where this is triggered from.
...
`progress.service.ts`, `dashboard.service.ts`, `notification.service.ts`),
the pure date/status logic (`lib/membership.ts`, `lib/date.ts`), the
authorization policies (`policies.ts`), and every Zod validation schema —
391 tests, using a mocked Prisma client, no database connection needed.
See "How trainer management works," "How workout & progress tracking
works," "How dashboards & analytics work," "How notifications work," "How
attendance works," and "How plans, memberships & payments work" below for
...
- **The member dashboard's "Notifications" card is a live preview**
  (the 3 most recent unread, with a link through to the full page), not
  a placeholder — see "How notifications work" below for Phase 8, which
  built the feature this card was originally a stand-in for.

## How notifications work

- **In-app only, by design — no email/push/worker/queue.** Every
  notification is a plain database row (`Notification`), created
  synchronously in the same request that causes it (a payment gets
  recorded, a workout plan gets created, a trainer gets assigned) or
  opportunistically during an existing self-healing read (see below for
  the two membership ones). There is no background job, no message
  queue, and no email infrastructure anywhere in this codebase to hang
  a "send an email too" step off of — building one wasn't asked for and
  would be exactly the kind of premature infra this project has avoided
  every phase (rate limiting is in-memory, there's no cron for the
  membership sweep, and so on). If email is ever added, the natural
  seam is `notification.service.ts`'s `createNotification` — a second
  side effect (enqueue/send an email) would be added at that one choke
  point, not scattered across every call site that creates a
  notification today.
- **`notification.service.ts`'s `createNotification` deliberately takes
  no `actor` parameter and does no authorization check** — unlike every
  other exported function in this codebase's service layer. There's no
  "actor" performing this action from the notified user's point of
  view; it's the system recording that something happened to them, the
  same way a `Payment` row's `recordedByUserId` or a `WorkoutPlan`'s
  `trainerId` isn't asking "is this allowed," because the surrounding
  action already answered that. It is never called from a page or
  Server Action directly, only from other services at the exact moment
  a notification-worthy event happens.
- **`canAccessNotification` is the one policy function with no admin
  override anywhere in this file.** Every other `canView*For` function
  gives an admin blanket access as an oversight capability; this one
  doesn't, because Phase 8's requirement ("users can only access their
  own notifications") is explicit and has no stated exception. This is
  enforced at the service layer (`markNotificationAsRead` checks it
  before touching a row) and tested directly — an admin trying to mark
  another user's notification as read gets the same `ForbiddenError` a
  different member would.
- **`createNotificationOnce` is the dedup guard behind "do not create
  excessive notifications."** It looks for an existing notification with
  the same recipient, type, and `relatedEntityId` before creating a new
  one. This matters specifically for the two membership-related types,
  which aren't created at a single, one-time event the way a payment or
  a new workout plan is — they're noticed during `syncMembershipStatus`,
  the same self-healing function `membership.service.ts` already runs
  on *every* read of a membership (a member's own membership page, their
  dashboard, an admin's sweep, ...). Without the dedup guard, visiting
  that page five times while a membership sits in its "expiring soon"
  window would create five identical notifications. A payment or a new
  workout plan, by contrast, is already a genuinely new, one-time
  database row each time it happens, so `createNotification` (no dedup)
  is used there instead — there's nothing to guard against.
- **The two membership notification types live inside
  `syncMembershipStatus`, not as a separate check somewhere else.**
  That function is the one place every membership read path already
  funnels through to keep `status` fresh (see "How plans, memberships &
  payments work"); adding the notification logic there means it's
  automatically exercised by `listMembershipsForMember`, `getMembership`,
  `createMembership`'s conflict check, `renewMembership`,
  `cancelMembership`, and the admin's `sweepMembershipStatuses` — one
  implementation, not six copies of "did this just expire." Notably,
  `sweepMembershipStatuses` was refactored in this phase to call
  `syncMembershipStatus` instead of its own inline status-update logic,
  specifically so a manual admin sweep also benefits from notification
  creation instead of being a second, notification-blind code path.
- **A trainer assignment notifies *both* sides, each with their own
  message** — the member ("You've been assigned a new trainer: ...")
  and the trainer ("You've been assigned a new member: ..."), the
  latter linking to that member's page on the trainer's roster
  (`/trainer/members/[id]`). Not deduped, since a fresh
  `TrainerAssignment` row is already a one-time event — re-assigning the
  same member to the same trainer is rejected as a conflict before
  either notification would ever be created a second time.
- **The notification bell and its unread badge live in `AppHeader`**,
  fed by `getUnreadNotificationCount(actor)` called once per request in
  each of the three role layouts (`admin/layout.tsx`,
  `trainer/layout.tsx`, `member/layout.tsx`) — the same place those
  layouts already call `requireRole()`. Every role gets its own
  `/[role]/notifications` page (`/admin/notifications`,
  `/trainer/notifications`, `/member/notifications`), each just a thin,
  role-gated wrapper around the same shared `NotificationList`
  component and the same underlying `listNotifications`/
  `markNotificationAsRead`/`markAllNotificationsAsRead` service
  functions — a notification looks and behaves identically regardless
  of which role is viewing it, so only the route's role gate differs
  between the three pages, matching the same "thin per-area page over a
  shared service" shape every other phase's role-specific pages use.
- **`NotificationType` is a small, closed enum** (`MEMBERSHIP_EXPIRING`,
  `MEMBERSHIP_EXPIRED`, `PAYMENT_RECORDED`, `WORKOUT_PLAN_ASSIGNED`,
  `TRAINER_ASSIGNED`), each with exactly one call site that creates it —
  not a generic free-text category a caller could misuse later to spam
...
- **No email, SMS, or push notifications** — everything is in-app only.
  There's no transactional email infrastructure in this codebase at
  all (see the earlier "no password reset flow" simplification, which
  has the same root cause), so there was nothing to wire a second
  delivery channel into yet. `notification.service.ts`'s
  `createNotification` is the one seam a future email channel would
  hook into — see "How notifications work."
- **No notification preferences.** A user can't opt out of a
  notification type or mute one; every event this phase generates
  always creates one. Given the small, deliberately curated set of
  events (five types total), this hasn't been a real problem yet.
- **No admin-facing notification management or broadcast tool.** An
  admin can't see other users' notifications (see "How notifications
  work" — there's no override, not even for admins) or send an ad-hoc
  announcement to a member/everyone. "Important admin/trainer updates"
  is covered narrowly today (a new trainer assignment) rather than a
  general-purpose broadcast feature.
- **No notification deletion**, only read/unread — matching this
  codebase's append-only convention for historical records (Payment,
  Attendance, ProgressLog all work the same way). A notification is a
  record that something happened, not a to-do list item to clear away.
- **The "expiring soon" window is a single fixed constant (3 days)**,
  not a per-plan or per-gym configurable setting. Changing it means
  changing `EXPIRING_SOON_THRESHOLD_DAYS` in `membership.service.ts`,
  not a UI setting.
- **Membership-related notifications depend on someone visiting a page
  that self-heals that membership** (see "How notifications work") —
  there's still no cron/background job in this codebase, so a
  membership that nobody looks at (the member never opens their
  membership page or dashboard, and no admin sweep runs) won't generate
  an "expiring soon" notification until someone finally does look. This
  is the same "opportunistic self-heal, not load-bearing automation"
  tradeoff `sweepMembershipStatuses` already made and documented before
  this phase existed.
...
src/
  app/
    ...
    admin/             layout.tsx (requireRole ADMIN) + dashboard/
      ...
      notifications/       own notifications: list, mark-as-read, mark-all-as-read
  ...
  components/
    ...
    dashboard/         StatCard, StatusBarChart (no charting library — see
                       "How dashboards & analytics work")
    notifications/     NotificationList — shared across all three role areas;
                       the bell + unread badge live in layout/app-header.tsx
                       instead, since every role's header shows one
  server/
    ...
    services/
      ...
      dashboard.service.ts           New, additive: role-scoped aggregate reads
                                    ...
                                    "How dashboards & analytics work"
      notification.service.ts        New, additive: createNotification/
                                    createNotificationOnce (called by other
                                    services, not actor-gated) + self-scoped
                                    list/count/markAsRead/markAllAsRead — see
                                    "How notifications work"
  lib/
...
prisma/
  schema.prisma        User, MemberProfile, MembershipPlan, Membership,
                       ...
                       WorkoutExercise, ProgressLog, Notification models;
                       Role/UserStatus/MembershipStatus/PaymentMethod/
                       PaymentStatus/AttendanceMethod/AssignmentStatus/
                       WorkoutPlanStatus/ProgressMetric/NotificationType enums
  seed.ts              Core test users + 22 member fixtures + 2 trainer
                       ...
                       a sample workout plan + sample progress logs (no
                       Notification rows — those are only ever created by
                       the services that generate them, see "How
                       notifications work")
